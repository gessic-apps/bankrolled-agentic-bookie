...
class RiskSimulator {
    ...
    simulateBets() {
        ...
        for (const user of this.userProfiles) {
            ...
            const numBets = Math.max(1, Math.round(this.config.avgBetsPerUser * (1 + (math.random(-0.5, 0.5) * user.variability))));
            for (let i = 0; i < numBets; i++) {
                // Pick a random time for this bet
                const betTime = new Date(startTime.getTime() + math.random() * (endTime.getTime() - startTime.getTime()));
                // User evaluates the odds for each outcome
                const perceivedValue = {};
                let bestOutcome = null;
                let highestValue = -Infinity;
                for (const outcome of this.outcomes) {
                    if (this.market.currentOdds[outcome] === undefined)
                        continue;
                ...
                if (bestOutcome && perceivedValue[bestOutcome] > 0.01) {
                    ...
                    let betSize = Math.min(baseBetSize * edgeFactor, maxBetLimit);
                    // Add some randomness
    ...
    runSingleSimulation() {
        ...
        if (actualOutcomeIndex === -1)
            actualOutcomeIndex = outcomesList.length - 1;
     ...
     */
    calculateOddsAdjustments(simResults) {
        ...
        for (const outcome of this.outcomes) {
            ...
            oddsAdjustments[outcome] = Math.max(-this.config.maxOddsVariation, Math.min(this.config.maxOddsVariation, baseAdjustment));
        ...
        if (totalExposure > maxAllowedExposure) {
            ...
            liquidityAdjustment = Math.max(-this.market.currentLiquidity * 0.2, // Don't reduce by more than 20%
     ...
     */
    calculateRiskMetrics(simResults) {
    ...
    runSimulation() {
        ...
        const { oddsAdjustments, betLimitAdjustments, liquidityAdjustment } = this.calculateOddsAdjustments(simulationResults);
        ...
        return {
            ...
            recommendedActions: {
                oddsAdjustments: oddsAdjustments,
                liquidityAdjustment,
                betLimitAdjustments: betLimitAdjustments
            },
            riskMetrics,
            simulationStats: {
                totalBets,
                avgBetSize,
                betOutcomeDistribution: betOutcomeDistribution,
                worstCaseScenario,
                bestCaseScenario
            }
        };
...
exports.RiskSimulator = RiskSimulator;
